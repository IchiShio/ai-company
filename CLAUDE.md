...
- **ホスティング**: GitHub Pages（リポジトリ: `IchiShio/ai-company`）
- **DNS**: A レコード → 185.199.108.153 等の GitHub Pages IPs
- **デプロイ**: `git push origin main` のみ（CI/CD なし、push 後に自動ビルド）
- **IchiShio/native-real**（Next.js）はアーカイブ済み・未使用
- **IchiShio/eikaiwa-hikaku** は旧リポジトリ名。現在は `ai-company` に移行済み
...
```bash
cd /Users/yusuke/projects/claude/ai-company
