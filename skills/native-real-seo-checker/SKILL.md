...
## Check 5: kioku-shinai 品質チェック（追加）

Section K のアクションが実行された場合、またはkioku-shinaiのファイルが変更された場合に実行する。

### チェック項目

| # | チェック内容 | PASS条件 | FAIL条件 |
|---|---|---|---|
| 5-1 | 単語データJSON整合性 | `WordData` 型に完全準拠 | 必須フィールド欠落、型不一致 |
| 5-2 | 音声ファイル存在確認 | JSONで参照される全音声が `public/audio/` に存在 | 音声ファイル欠落 |
| 5-3 | 穴埋め答えバレ防止 | 穴埋め形式の選択肢に正解単語がそのまま含まれていない、またはUIで単語名を非表示にしている | 答えがバレる状態 |
| 5-4 | ビルド成功 | `npx next build` がエラーなし | ビルドエラー |
| 5-5 | fact-checker通過 | 新規/変更された単語データが `kioku-shinai-fact-checker` で検証済み | 未検証のデータがコミットされた |

### 実行手順

```
# 5-1: JSON整合性
Globツールで ~/projects/claude/kioku-shinai/public/data/*.json を一覧取得し、
各ファイルをReadツールで読んで `word` フィールドの存在を確認する。
（`node -e` は使用しない。Readツール + Claude判定で代替）

# 5-2: 音声ファイル確認
Globツールで ~/projects/claude/kioku-shinai/public/audio/*/word.mp3 を確認
...
- 5-1〜5-3: executorに修正を指示して再実行
- 5-4: ビルドエラーを解析して修正
- 5-5: `kioku-shinai-fact-checker` を実行してから再コミット
- 2回FAIL → buchoにエスカレーション
