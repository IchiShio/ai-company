...
**メタタグ検査**:
1. `<title>` が60文字以内か（Googleに公式規定なし・表示幅からの実用目安）
2. `<meta name="description">` が存在するか（Googleに文字数制限なし・内容の質を重視）
...
**構造化データ検査**:
1. JSON-LDに `datePublished` が含まれるか（Google推奨プロパティ・必須ではない）
2. JSON-LDに `dateModified` が含まれるか（Google推奨プロパティ・必須ではない）
3. JSON-LDに `author` (`"@type": "Person"` または `"Organization"`) が含まれるか（Google推奨）
4. ※ FAQPage スキーマは Google公式方針上「政府・医療機関サイト限定」のためnative-realでは推奨しない
