...
❌ FAIL が出たら自動リカバリーを試み、失敗なら `native-real-bucho` にエスカレーションする。
...
- ❌ FAIL → **自動リカバリーを試みる**（失敗ソースの再取得 → 再チェック）
  - リカバリー成功 → Step 2 へ進む
  - リカバリー失敗 → `native-real-bucho` にエスカレーション
...
- ❌ FAIL → **自動リカバリーを試みる**（欠落セクション再分析 / Top5再生成 → 再チェック）
  - リカバリー成功 → Step 3 へ進む
  - リカバリー失敗 → `native-real-bucho` にエスカレーション
...
- ❌ FAIL → **自動リカバリーを試みる**（未実行アクション再実行 / 品質修正 / push再試行 → 再チェック）
  - リカバリー成功 → 完了レポートを表示
  - リカバリー失敗 → `native-real-bucho` にエスカレーション
