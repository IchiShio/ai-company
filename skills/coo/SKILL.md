...
description: |
  ...
  native-real部門・X部門（@ichi_eigo）のKPI集約、予算・コスト追跡、
...
| schedule.json 残 ≤ 6件 | バッチ補充が必要 | `/x-bucho post` → schedule.json更新 → push |
| 3日以上データ未収集 | パフォーマンス取得が必要 | `x-data-collector` を実行 |
| 投稿データが10件以上溜まった | 週次レビューの時期 | `/x-bucho review` を実行 |
...
**⚠️ 完全自動モード（厳守）:**
- 社長への確認・承認は一切不要。全タスクを自動で連続実行する
- 「次のタスクに進みますか？」「確認してください」などの停止は禁止
