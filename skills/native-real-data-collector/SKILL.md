...
4. **ZIPを展開して必要なファイルを取り出す**:
   ...
   - ZIPを展開すると以下のCSVが入っている:
     - クエリ.csv → `search_console_queries.csv` にリネーム
     - ページ.csv → `search_console_pages.csv` にリネーム
     - デバイス.csv → `search_console_devices.csv` にリネーム
     - 国.csv → `search_console_countries.csv` にリネーム
     - 日付チャート.csv → `search_console_dates.csv` にリネーム
...
1. **トラフィック獲得レポートの取得**:
   ...
   - このファイルを `ga4_traffic_acquisition.csv` としてリネーム・保存する
...
1. `site-explorer-metrics` → `ahrefs_overview.csv`
...
```
/Users/yusuke/マイドライブ/02_AI・ブログ・仕事/GoogleG4,SearchConsole/
```

**Google Drive上の場所:**
- マイドライブ → 02_AI・ブログ・仕事 → GoogleG4,SearchConsole
...
└── YYYY-MM-DD/
    ...
    ├── search_console_devices.csv
    ├── search_console_countries.csv
    ├── search_console_dates.csv
...
1. Google Drive同期フォルダ `/Users/yusuke/マイドライブ/02_AI・ブログ・仕事/GoogleG4,SearchConsole/` にアクセスする
   - Coworkセッションでは `request_cowork_directory` で `~/マイドライブ` をマウントする
   - マウント後のVMパスは `/sessions/.../mnt/マイドライブ/02_AI・ブログ・仕事/GoogleG4,SearchConsole/`
